...
async def on_raw_reaction_add(payload):
  ...
  if user != message.author and channel == ticket:
    ...
    if str(reaction) == "🆘":
      ...
      embed = discord.Embed(description = '〽️ Para adicionar outra pessoa no ticket, use o comando !adc <id>\n\n〽️ Para remover uma pessoa do ticket, use o comando !rem <id>\n\n〽️ Para finalizar o ticket, use o comando !fechar', color = discord.Color.orange())
      embed.set_footer(text = '🦅 | Okabam Roleplay')
      my_msg = await channel_ticket.send(user.mention, embed = embed)
      
      await channel_ticket.set_permissions(discord.utils.get(guild.roles, name="STAFF"), read_messages=True)
      await channel_ticket.set_permissions(discord.utils.get(guild.roles,  name="SUPORTE"), read_messages=True)

    if str(reaction) == "⚠️":
      repN = len(discord.utils.get(guild.categories, name='Denúncias').channels) + 1

      channel_report = await guild.create_text_channel(f'🛑│𝙳𝚎𝚗𝚞𝚗𝚌𝚒𝚊 n° {repN}', overwrites = overwrites, category = discord.utils.get(guild.categories, name='Denúncias'))
      ...
      await channel_report.set_permissions(discord.utils.get(guild.roles, name="STAFF"), read_messages=True)
      await channel_report.set_permissions(discord.utils.get(guild.roles,  name="SUPORTE"), read_messages=True)  
